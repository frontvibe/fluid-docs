...
Move into the project directory `cd ./path-to-your-app` and create a new Sanity project running the following command:

<Code lang="sh" code={"npx sanity@latest init --env .env.sanity"} />

This command will create a new Sanity project and generate a `.env.sanity` file in your project directory with your `SANITY_PROJECT_ID` and `SANITY_DATASET`.
